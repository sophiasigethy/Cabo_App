...
public class MainActivity extends AppCompatActivity {
    ...
    private void handelTextMessage(String message) throws JSONException {
        ...
        if (jsonObject.has("Hallo")) {
            mMessage = TypeDefs.server + jsonObject.get("Hallo").toString();
            showText();
        }
        ...
        if (jsonObject.has("Welcome")) {
            ...
            if (welcome.has("Player")){
                ...
                mMessage = TypeDefs.server +"Hello "+ player.getName()+ " with id: " + player.getId();
        ...
        if (jsonObject.has("newPlayer")) {
            mMessage = TypeDefs.server +jsonObject.get("newPlayer").toString();
            showText();
        }
        ...
        if (jsonObject.has("usernameInUse")) {
            ...
            mMessage= TypeDefs.server +name + " is already in use. Please state another username.";
            showText();
        }
        ...
        if (jsonObject.has("sendOtherPlayer")) {
            ...
            if (js.has("player")){
                ...
                if (player.getId()!=me.getId()){
                    mMessage = TypeDefs.server+ player.getName()+ " with id: " + player.getId() + "has already entered the game.";
                    players.add(player);
                    showText();
                }
            }
        ...
        if (jsonObject.has("chatMessage")) {
            mMessage = jsonObject.get("chatMessage").toString();
            showText();
        }
